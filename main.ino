...
void whileloop()
{
    ...
    if (acceleration < MAXSPEED)
        ...
        motorspeed = MAXSPEED, acceleration = MAXSPEED;
...
void sendMQTTMessage()
{
    ...
    if (status == 4)
        snprintf(blindstate, 8, "STOPPED"), itoa(100, blindposition, 10);
    else if (status == 3)
        snprintf(blindstate, 11, "INCREASING"), itoa(travelPercent, blindposition, 10);
    else if (status == 2)
        snprintf(blindstate, 8, "STOPPED"), itoa(travelPercent, blindposition, 10);
    else if (status == 1)
        snprintf(blindstate, 11, "DECREASING"), itoa(travelPercent, blindposition, 10);
    else if (status == 0)
        snprintf(blindstate, 8, "STOPPED"), itoa(0, blindposition, 10);
...
void downButtonCheck()
{
    if (digitalRead(DOWNBUTTONPIN) == HIGH)
    {
        ...
        if ((millis() - buttonTimer > LONGPRESSTIME) && (DownLongPressActive == false))
        { // Button Held
...
void setup()
{
    ...
    if (ENCODERWIRE == 0)
        Serial.println("UP = CCW || DOWN = CW"), flash_LED(1, String("Yellow"));
    else
    {
#if DEBUG
